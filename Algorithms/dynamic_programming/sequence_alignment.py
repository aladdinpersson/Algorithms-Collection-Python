...
class SequenceAlignment(object):
    ...
    def find_solution(self, OPT, m, n):
        ...
        align = OPT[m-1][n-1] + self.delta(self.x, self.y, m-1,n-1) if m != 0 and n != 0 else float('inf')
    ...
    def alignment(self):
        ...
        for i in range(1,m+1):
            for j in range(1,n+1):
                OPT[i][j] = min(OPT[i-1][j-1] + self.delta(self.x,self.y,i-1,j-1), OPT[i-1][j] + 1, OPT[i][j-1] + 1) #align, delete, insert respectively
